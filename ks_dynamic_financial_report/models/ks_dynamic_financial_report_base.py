...
class ks_dynamic_financial_base(models.Model):
    ...
    def ks_process_aging_data(self, ks_df_informations, offset=0, ks_partner=0, fetch_range=FETCH_RANGE):
        ...
        if ks_partner:
            ...
            lang = self.env.user.lang
            journal_name = f"COALESCE(j.name->>'{lang}')" if \
                self.pool['account.journal'].name.translate else 'j.name'

            SELECT = f"""SELECT m.name AS move_name,
                                m.id AS move_id,
                                l.date AS date,
    ...
    def ks_build_consolidate_query(self, ks_df_informations):
        lang = self.env.user.lang
        account_name = f"COALESCE(account.name->>'{lang}')" if \
        ...
        select = f"""
                SELECT to_char("account_move_line".date, 'MM') as month,
                       ...
                       j.id as journal_id,
                       {journal_name} as journal_name, j.code as journal_code,
                       {account_name} as account_name, account.code as account_code,
