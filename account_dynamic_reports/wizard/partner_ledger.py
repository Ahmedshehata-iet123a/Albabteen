...
class InsPartnerLedger(models.TransientModel):
    ...
    def process_data(self):
        ...
        for line in partner_ids:
            ...
            if account_move_line:
                ids.append(line.id)
        if self.all_move:
            partner_ids = partner_ids.search([('id', 'in', ids)])
        move_lines = {
            x.id: {
                'name': x.name,
                'code': x.id,
        ...
        for partner in partner_ids:
            ...
            for row in cr.dictfetchall():
                ...
                if self.all_move == False and ((data.get('display_accounts') == 'balance_not_zero' and currency.is_zero(
    ...
    def action_xlsx(self):
        ...
        row_pos_2 = 0
        row_pos = 0
        sheet.merge_range(row_pos, 0, row_pos + 2, 8, 'Partner Ledger' + ' - ' + data['company_id'][1], format_title)

        product_image = io.BytesIO(base64.b64decode(self.env.user.company_id.logo))
        sheet.insert_image(row_pos, 4, "image.png", {'image_data': product_image, 'x_scale': 0.125, 'y_scale': 0.0625})
        row_pos += 3        # Write filters

        sheet.write(row_pos, 0, _('Date From'), format_header)
        datestring = fields.Date.from_string(str(filter['date_from'])).strftime(lang_id.date_format)
        sheet.write(row_pos, 1, datestring or '', content_header_date)
        row_pos += 1
        sheet.write(row_pos, 0, _('Date to'), format_header)
        datestring = fields.Date.from_string(str(filter['date_to'])).strftime(lang_id.date_format)
        sheet.write(row_pos, 1, datestring or '', content_header_date)
        row_pos += 1
        sheet.write(row_pos, 0, _('Target moves'), format_header)
        sheet.write(row_pos, 1, filter['target_moves'], content_header)
        row_pos += 1
        sheet.write(row_pos, 0, _('Display accounts'), format_header)
        sheet.write(row_pos, 1, filter['display_accounts'], content_header)
        row_pos += 1
        sheet.write(row_pos, 0, _('Reconciled'), format_header)
        sheet.write(row_pos, 1, filter['reconciled'], content_header)
        row_pos += 1
        sheet.write(row_pos, 0, _('Initial Balance'), format_header)
        sheet.write(row_pos, 1, filter['initial_balance'], content_header)
        row_pos += 1
        # Journals
        row_pos += 2
        sheet.write(row_pos, 0, _('Journals'), format_header)
        j_list = ', '.join([lt or '' for lt in filter.get('journals')])
        sheet.write(row_pos, 1, j_list, content_header)
        # Partners
        row_pos += 1
        sheet.write(row_pos, 0, _('Partners'), format_header)
        p_list = ', '.join([lt or '' for lt in filter.get('partners')])
        sheet.write(row_pos, 1, p_list, content_header)
        # Partner Tags
        row_pos += 1
        sheet.write(row_pos, 0, _('Partner Tag'), format_header)
        p_list = ', '.join([lt or '' for lt in filter.get('categories')])
        sheet.write(row_pos, 1, p_list, content_header)
        # Accounts
        row_pos += 1
        sheet.write(row_pos, 0, _('Accounts'), format_header)
        a_list = ', '.join([lt or '' for lt in filter.get('accounts')])
        sheet.write(row_pos, 1, a_list, content_header)
        ...
        if account_lines:
            for line in account_lines:
                ...
                sheet.write(row_pos, 5, '{:,.2f}'.format(float(account_lines[line].get('debit'))), line_header)
                sheet.write(row_pos, 6, '{:,.2f}'.format(float(account_lines[line].get('credit'))), line_header)
                sheet.write(row_pos, 7, '{:,.2f}'.format(float(account_lines[line].get('balance'))), line_header)
                ...
                if filter.get('include_details', False):
                    ...
                    for sub_line in sub_lines:
                        if sub_line.get('move_name') == 'Initial Balance':
                            ...
                            sheet.write(row_pos, 5, '{:,.2f}'.format(float(sub_line.get('debit'))),
                                        line_header_light_initial)
                            sheet.write(row_pos, 6, '{:,.2f}'.format(float(sub_line.get('credit'))),
                                        line_header_light_initial)
                            sheet.write(row_pos, 7, '{:,.2f}'.format(float(sub_line.get('balance'))),
                                        line_header_light_initial)
                            ...
                            sheet.write(row_pos, 1, sub_line.get('lcode'),
                                        line_header_light)
                            sheet.write(row_pos, 2, sub_line.get('account_name')[self.env.user.lang] or '',
                                        line_header_light)
                            ...
                            sheet.write(row_pos, 5, '{:,.2f}'.format(float(account_lines[line].get('debit'))),
                                        line_header_light_ending)
                            sheet.write(row_pos, 6, '{:,.2f}'.format(float(account_lines[line].get('credit'))),
                                        line_header_light_ending)
                            sheet.write(row_pos, 7, '{:,.2f}'.format(float(account_lines[line].get('balance'))),
                                        line_header_light_ending)
