...
class InsTrialBalance(models.TransientModel):
    ...
    def process_data(self, data):
        if data:
            ...
            if self.has_move:
                self.display_accounts = 'all'
                ids = []
                for rec in account_ids:
                    account_move_line = self.env['account.move.line'].search([('account_id', '=', rec.id)], limit=1)
                    if account_move_line:
                        ids.append(rec.id)
                account_ids = self.env['account.account'].search([('id', 'in', ids)])
            ...
            for account in account_ids:
                ...
                #     WHERE_INIT += " AND a.account_type not in  ( 'asset_receivable','asset_cash','asset_current','asset_non_current', 'asset_prepayments','asset_fixed','liability_payable', 'liability_credit_card', 'liability_current','liability_non_current', 'equity','equity_unaffected')"
                WHERE_INIT += " AND l.account_id = %s" % account.id
                init_blns = 0.0
                deb = 0.0
                cre = 0.0
            ...
            if self.strict_range:
                total_init_cre += retained_credit
                total_init_deb += retained_debit
                total_init_bal += retained_earnings
    ...
    def action_xlsx(self):
        ...
        row_pos_2 = 0
        row_pos = 0
        sheet.merge_range(0, 0, 2, 3, 'Trial Balance' + ' - ' + data['company_id'][1], format_title)
        product_image = io.BytesIO(base64.b64decode(self.env.user.company_id.logo))
        sheet.insert_image(0, 4, "image.png", {'image_data': product_image, 'x_scale': 0.125, 'y_scale': 0.0625})
        ...
        if account_lines:
            if not filter.get('show_hierarchy'):
                for line in account_lines:  # Normal lines
                    ...
                    sheet.write(row_pos, 1, '{:,.2f}'.format(float(account_lines[line].get('initial_debit'))),
                                line_header_light)
                    sheet.write(row_pos, 2, '{:,.2f}'.format(float(account_lines[line].get('initial_credit'))),
                                line_header_light)
                    sheet.write(row_pos, 3, '{:,.2f}'.format(float(account_lines[line].get('initial_balance'))),
                                line_header_highlight)
                    sheet.write(row_pos, 4, '{:,.2f}'.format(float(account_lines[line].get('debit'))),
                                line_header_light)
                    sheet.write(row_pos, 5, '{:,.2f}'.format(float(account_lines[line].get('credit'))),
                                line_header_light)
                    sheet.write(row_pos, 6, '{:,.2f}'.format(float(account_lines[line].get('balance'))),
                                line_header_highlight)
                    sheet.write(row_pos, 7, '{:,.2f}'.format(float(account_lines[line].get('ending_debit'))),
                                line_header_light)
                    sheet.write(row_pos, 8, '{:,.2f}'.format(float(account_lines[line].get('ending_credit'))),
                                line_header_light)
                    sheet.write(row_pos, 9, '{:,.2f}'.format(float(account_lines[line].get('ending_balance'))),
                                line_header_highlight)
            else:
                for line in account_lines:  # Normal lines
                    row_pos += 1
                    ...
                    if line.get('dummy'):
                        ...
                        sheet.write(row_pos, 0,
                                    ...
                                    line_header_light_left)
                    sheet.write(row_pos, 1, '{:,.2f}'.format(float(line.get('initial_debit'))), line_header_light)
                    sheet.write(row_pos, 2, '{:,.2f}'.format(float(line.get('initial_credit'))), line_header_light)
                    sheet.write(row_pos, 3, '{:,.2f}'.format(float(line.get('initial_balance'))),
                                line_header_highlight)
                    sheet.write(row_pos, 4, '{:,.2f}'.format(float(line.get('debit'))), line_header_light)
                    sheet.write(row_pos, 5, '{:,.2f}'.format(float(line.get('credit'))), line_header_light)
                    sheet.write(row_pos, 6, '{:,.2f}'.format(float(line.get('balance'))), line_header_highlight)
                    sheet.write(row_pos, 7, '{:,.2f}'.format(float(line.get('ending_debit'))), line_header_light)
                    sheet.write(row_pos, 8, '{:,.2f}'.format(float(line.get('ending_credit'))), line_header_light)
                    sheet.write(row_pos, 9, '{:,.2f}'.format(float(line.get('ending_balance'))),
                                line_header_highlight)
            ...
            if filter.get('strict_range'):
                ...
                sheet.write(row_pos, 0, '        ' + retained['RETAINED'].get('name'),
                            line_header_light_left)
                sheet.write(row_pos, 1, '{:,.2f}'.format(float(retained['RETAINED'].get('initial_debit'))),
                            line_header_light)
                sheet.write(row_pos, 2, '{:,.2f}'.format(float(retained['RETAINED'].get('initial_credit'))),
                            line_header_light)
                sheet.write(row_pos, 3, '{:,.2f}'.format(float(retained['RETAINED'].get('initial_balance'))),
                            line_header_highlight)
                sheet.write(row_pos, 4, '{:,.2f}'.format(float(retained['RETAINED'].get('debit'))),
                            line_header_light)
                sheet.write(row_pos, 5, '{:,.2f}'.format(float(retained['RETAINED'].get('credit'))),
                            line_header_light)
                sheet.write(row_pos, 6, '{:,.2f}'.format(float(retained['RETAINED'].get('balance'))),
                            line_header_highlight)
                sheet.write(row_pos, 7, '{:,.2f}'.format(float(retained['RETAINED'].get('ending_debit'))),
                            line_header_light)
                sheet.write(row_pos, 8, '{:,.2f}'.format(float(retained['RETAINED'].get('ending_credit'))),
                            line_header_light)
                sheet.write(row_pos, 9, '{:,.2f}'.format(float(retained['RETAINED'].get('ending_balance'))),
                            line_header_highlight)
            # Sub total line
            row_pos += 2
            sheet.write(row_pos, 0,
            ...
            sheet.write(row_pos, 1, '{:,.2f}'.format(float(subtotal['SUBTOTAL'].get('initial_debit'))),
                        line_header_light_total)
            sheet.write(row_pos, 2, '{:,.2f}'.format(float(subtotal['SUBTOTAL'].get('initial_credit'))),
                        line_header_light_total)
            sheet.write(row_pos, 3, '{:,.2f}'.format(float(subtotal['SUBTOTAL'].get('initial_balance'))),
                        line_header_total)
            sheet.write(row_pos, 4, '{:,.2f}'.format(float(subtotal['SUBTOTAL'].get('debit'))),
                        line_header_light_total)
            sheet.write(row_pos, 5, '{:,.2f}'.format(float(subtotal['SUBTOTAL'].get('credit'))),
                        line_header_light_total)
            sheet.write(row_pos, 6, '{:,.2f}'.format(float(subtotal['SUBTOTAL'].get('balance'))), line_header_total)
            sheet.write(row_pos, 7, '{:,.2f}'.format(float(subtotal['SUBTOTAL'].get('ending_debit'))),
                        line_header_light_total)
            sheet.write(row_pos, 8, '{:,.2f}'.format(float(subtotal['SUBTOTAL'].get('ending_credit'))),
                        line_header_light_total)
            sheet.write(row_pos, 9, '{:,.2f}'.format(float(subtotal['SUBTOTAL'].get('ending_balance'))),
                        line_header_total)
