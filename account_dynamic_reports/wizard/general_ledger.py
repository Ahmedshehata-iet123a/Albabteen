...
class InsGeneralLedger(models.TransientModel):
    ...
    display_accounts = fields.Selection(
        [('all', 'All'),
         ('balance_not_zero', 'With balance not equal to zero'),
         ('with_move', 'Have Moves')
    ...
    def process_filters(self):
        ...
        if data.get('account_tag_ids', []):
            filters['account_tags'] = self.env['account.account.tag'].browse(data.get('account_tag_ids', [])).mapped(
                'name')
        else:
        ...
        elif data.get('display_accounts') == 'with_move':
            filters['display_accounts'] = 'Have Moves'
        else:
    ...
    def process_data(self):
        ...
        acc_ids = []
        if data.get('display_accounts') == 'with_move':
            for account in account_ids:
                move_lines_ids = self.env['account.move.line'].search([('account_id', '=', account.id)], limit=1)

                if move_lines_ids:
                    acc_ids.append(account.id)
            account_ids = self.env['account.account'].search([('id', 'in', acc_ids)])
    ...
    def action_xlsx(self):
        ...
        row_pos_2 = 0
        row_pos = 0
        sheet.merge_range(0, 0, 2, 3, 'General Ledger' + ' - ' + data['company_id'][1], format_title)
                ...
                if filter.get('include_details', False):
                    ...
                    for sub_line in sub_lines:
                        if sub_line.get('move_name') == 'Initial Balance':
                            ...
                            sheet.write(row_pos, 5, '{:,.2f}'.format(float(sub_line.get('debit'))), line_header_light_initial)
                            sheet.write(row_pos, 6, '{:,.2f}'.format(float(sub_line.get('credit'))), line_header_light_initial)
                            sheet.write(row_pos, 7, '{:,.2f}'.format(float(sub_line.get('balance'))), line_header_light_initial)
                            ...
                            sheet.write(row_pos, 5, '{:,.2f}'.format(float(sub_line.get('debit'))), line_header_light)
                            sheet.write(row_pos, 6, '{:,.2f}'.format(float(sub_line.get('credit'))), line_header_light)
                            sheet.write(row_pos, 7, '{:,.2f}'.format(float(sub_line.get('balance'))), line_header_light)
                        else:  # Ending Balance
                            row_pos += 1
                            sheet.write(row_pos, 4, sub_line.get('move_name'), line_header_light_ending_bold)
                            sheet.write(row_pos, 5, '{:,.2f}'.format(float(account_lines[line].get('debit'))), line_header_light_ending)
                            sheet.write(row_pos, 6, '{:,.2f}'.format(float(account_lines[line].get('credit'))), line_header_light_ending)
                            sheet.write(row_pos, 7, '{:,.2f}'.format(float(account_lines[line].get('balance'))), line_header_light_ending)
